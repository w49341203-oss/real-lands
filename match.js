...
function normalize(input){
 ...
 let players=(src.players||[]).map((p,i)=>({controller:CONTROLLERS.includes(p.controller)?p.controller:'ai',nation:typeof p.nation==='string'&&p.nation?p.nation:map.id,teamId:p.teamId|0||i+1,difficulty:DIFFICULTIES.includes(p.difficulty)?p.difficulty:'normal',color:p.color||null,name:p.name||null,passive:p.passive===true,/* 旁觀勢力：同時期沒參戰的國家，開局與所有人停戰、AI 不會主動開戰（Helen：只有兩方太無聊） */spawn:p.spawn&&Number.isFinite(+p.spawn.lon)&&Number.isFinite(+p.spawn.lat)?{lon:+p.spawn.lon,lat:+p.spawn.lat}:null}));
